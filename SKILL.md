...
3. 若存在不确定项，先调用可用搜索技能/工具核实关键事实（本地搜索优先，必要时 web search）。
4. 判断是否缺少最小必要信息（最多追问 1 个关键问题）。
5. 生成“保留原意、可执行”的查询语句。
6. 执行 `python3 main.py --query "..."`。
7. 将结果整理成用户能直接消费的中文答案（多子任务分段返回）。
